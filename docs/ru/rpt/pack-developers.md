...
Теперь в каждом *json* файле вы можете указать поле `rpt`, в котором указываются флаги и переменные в формате, указанном в блоке кода ниже. Их можно читать через определённые типы моделей. Об этом вы можете прочитать [здесь](work-with-variables.md).
